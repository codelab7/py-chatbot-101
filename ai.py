...
GEMINI_API_KEY = "AIzaSyCEmdgQd6FgAgQEbNZi1gNg1NI2t77ukYU"
...
def register(body: RegisterRequest, db=Depends(get_db)):
    ...
    welcome = "Welcome! I'm AstraBot 🤖 Powered by Gemini AI. Ask me anything!"
    cur.execute(
...
def create_chat(body: CreateChatRequest, current_user=Depends(get_current_user), db=Depends(get_db)):
    ...
    db.commit()
    chat_id = cur.lastrowid
    return {"message": "Chat created", "chat": {"id": chat_id, "title": body.title}}
...
async def send_message(chat_id: int, body: SendMessageRequest, current_user=Depends(get_current_user), db=Depends(get_db)):
    user_id = int(current_user["sub"])
    cur = db.cursor(dictionary=True)
    ...
    # છેલ્લા 10 messages history fetch
    cur.execute("""
